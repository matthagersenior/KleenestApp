...
(function(){'use strict';
 ...
 const names=['KleenestSupabase','KleenestRuntime','KleenestBusinessData','KleenestPartnershipUI','kleenestAcceptance','KleenestSessionBridge','kleenestLegacyAuthGuard'];
 ...
 api.check=async function(){const s=api.snapshot();if(!s.ready)return s;try{const session=await window.KleenestRuntime.getSession();return {...s,sessionAuthenticated:!!session,ok:s.ok};}catch(e){return {...s,ok:false,error:e?.message||String(e)};}};
