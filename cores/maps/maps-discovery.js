/* Kleenest Maps Discovery Core — authoritative Supabase + live public-data discovery. */
export function createMapsDiscovery({supabase}={}){
 ...
 async function refresh({filters={},position=null,radiusMeters=15000,limit=100}={}){
  ...
  const publicRows=Number.isFinite(lat)&&Number.isFinite(lng)?await fetchOsm({lat,lng,radiusMeters:safeRadius,limit:safeLimit}).catch(e=>{console.warn('[Maps] public discovery failed',e);return []}):[];
  const merged=mergeUnique(dbRows,publicRows,safeLimit);
