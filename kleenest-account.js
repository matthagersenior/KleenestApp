...
(function () {
  ...
  window.KleenestAccount.notifications = async function (limit=50) {
    await auth();
    const {data,error}=await client().rpc('user_notifications',{p_limit:limit});
    if(error) throw error; return data||[];
  };

  window.KleenestAccount.markNotificationRead = async function (notificationId) {
    await auth();
    return window.KleenestRuntime.markNotificationRead(notificationId);
  };

  window.KleenestAccount.subscription = async function () {
    await auth();
