...
(function(){'use strict';
 ...
 function block(e){
  if(!liveAuth())return;
  // Once the modular auth UI is installed, it is the sole owner of auth
  // controls. The legacy guard must not swallow its click before the
  // canonical handler receives it.
  if(window.KleenestRuntime?.__authUiInstalled)return;
