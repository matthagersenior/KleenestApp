...
(function(){'use strict';
  ...
  api.mount=function(detail){
    ...
    if(!target){
      // Business UI is intentionally scoped to the Business workspace.
      // Never append business content to document.body or another tab.
      api.clear();
      return;
    }
    ...
    h.innerHTML='<div class="kleenest-business-live-inner"><h3>Live Business Data</h3>'+bs.map(b=>{
      const d=b.dashboard||{},l=b.locations||[],r=b.reviews||[],p=b.promotions||[],m=b.membership||{};
      return '<article class="kleenest-business-card" data-business-id="'+esc(b.businessId)+'"><div><strong>'+esc(d.business_name||d.name||m.business_name||'Business')+'</strong><span> · '+esc(m.role||'member')+'</span></div><div class="kleenest-business-stats"><span>Locations: '+l.length+'</span><span>Reviews: '+r.length+'</span><span>Promotions: '+p.length+'</span></div></article>';
    }).join('')+'</div>';
  };
