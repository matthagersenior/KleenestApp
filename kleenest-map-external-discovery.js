...
(function(){'use strict';
 ...
 async function query(q){for(const host of ['https://overpass-api.de/api/interpreter','https://overpass.kumi.systems/api/interpreter']){try{const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),12000);const res=await fetch(host+'?data='+encodeURIComponent(q),{headers:{Accept:'application/json'},signal:ctl.signal});clearTimeout(timer);if(!res.ok)continue;const data=await res.json();if(Array.isArray(data.elements))return data}catch(e){console.warn('[Kleenest] Overpass host failed',host,e)}}return {elements:[]};}
 function classify(t){
 ...
 X.nearby=async(lat,lng,radiusMeters=10000)=>{
  const r=Math.min(Math.max(Number(radiusMeters)||10000,1000),20000),union=filters.map(f=>`nwr${f}(around:${r},${Number(lat)},${Number(lng)});`).join('');
  const data=await query(`[out:json][timeout:12];(${union});out center tags;`),seen=new Set(),out=[];
