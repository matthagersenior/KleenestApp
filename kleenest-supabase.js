...
(function(){'use strict';
 ...
 async function signIn(email,password){const {data,error}=await getClient().auth.signInWithPassword({email:String(email).trim(),password});if(error)throw error;return data;}
 ...
 async function profile(){try{return await rpc('get_my_profile',{});}catch(e){return null;}}
 ...
 window.KleenestSupabase={getClient,client:getClient,session,signIn,signUp,ensureProfile,profile,signOut,rpc,nearbyLocations,nearby_locations:nearbyLocations,businessMemberships};
})();