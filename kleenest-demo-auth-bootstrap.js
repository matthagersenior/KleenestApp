...
(function(){'use strict';
 ...
 async function complete(key){const c=client();const {data,error}=await c.rpc('demo_complete_identity',{p_demo_key:key});if(error)throw error;return data;}
 async function create(key,password){const a=accounts.find(x=>x.key===key);if(!a)throw new Error('Unknown demo account');const c=client();const p=String(password||'');if(p.length<8)throw new Error('Use a password of at least 8 characters');const r=await c.auth.signUp({email:a.email,password:p,options:{data:{demo_key:a.key,display_name:a.name,username:a.username,subscription_tier:a.tier,is_demo_test:true}}});if(r.error)throw r.error;if(r.data.session)await complete(a.key);return {account:a,session:!!r.data.session,userId:r.data.user?.id||null,confirmationRequired:!r.data.session};}
 async function signIn(key,password){const a=accounts.find(x=>x.key===key);if(!a)throw new Error('Unknown demo account');const c=client();const r=await c.auth.signInWithPassword({email:a.email,password});if(r.error)throw r.error;await complete(a.key);return r.data;}
