...
(function(){'use strict';
  ...
  function activate(rawView){
    ...
    document.querySelectorAll('[data-kleenest-nav]').forEach(el=>{const active=normalize(el.dataset.kleenestNav)===view;el.classList.toggle('active',active);el.setAttribute('aria-current',active?'page':'false');});
  ...
  function init(){
    ...
    document.addEventListener('click',e=>{const el=e.target.closest?.('[data-kleenest-nav],.nav-btn,.bottom-btn,[data-page],[data-view]');if(!el)return;const view=inferNavView(el);if(!view)return;e.preventDefault();e.stopPropagation();activate(view);},true);
    const initial=document.querySelector('[data-kleenest-nav].active')?.dataset.kleenestNav||document.querySelector('[data-kleenest-view]')?.dataset.kleenestView||'home';activate(initial);
  }
