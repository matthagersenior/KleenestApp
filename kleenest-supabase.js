...
(function(){'use strict';
 const url='https://ssgesjzdvdsqacdtasje.supabase.co'; const publishableKey='sb_publishable_f1rbczgvWKSQy2g9mTDQZg_K9Wv19bL';
 const redirect=()=>window.location.origin+window.location.pathname;
 function getClient(){if(window.supabase?.createClient){if(!window.KleenestSupabaseClient)window.KleenestSupabaseClient=window.supabase.createClient(url,publishableKey,{auth:{autoRefreshToken:true,persistSession:true,detectSessionInUrl:true,flowType:'pkce'}});return window.KleenestSupabaseClient;}throw new Error('Supabase JavaScript client is not loaded');}
 ...
 async function signUp(email,password,metadata){const {data,error}=await getClient().auth.signUp({email,password,options:{data:metadata||{},emailRedirectTo:redirect()}});if(error)throw error;return data;}
 ...
 async function provision(){const s=await session();if(!s)return null;const p=await ensureProfile();return {session:s,profile:p};}
