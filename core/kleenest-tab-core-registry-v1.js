...
const specs={
 home:()=>import('../cores/home/home-core.js?tabcore=18'),
 maps:()=>import('../cores/maps/maps-tab-core.js?tabcore=18'),
 social:()=>import('../cores/social/social-tab-core.js?tabcore=18'),
 profile:()=>import('../cores/profile/profile-core.js?tabcore=18'),
 business:()=>import('../cores/business/business-core.js?tabcore=18'),
 admin:()=>import('../cores/admin/admin-core.js?tabcore=18')
};
...
async function mount(name,container,context={}){
 ...
 if(existing){if(existing.container===container)return existing.cleanup;throw new Error('Canonical tab core already mounted: '+name)}
 const inFlight=pending.get(name);
 if(inFlight){const mounted=await inFlight;if(mounted.container!==container)throw new Error('Canonical tab core mount already in progress: '+name);return mounted.cleanup}
 const operation=(async()=>{
  ...
  await core.mount();let released=false;
  const release=async()=>{if(released)return;released=true;try{await core.destroy?.()}finally{instances.delete(name)}};
  const mounted={name,container,core,cleanup:release};instances.set(name,mounted);return mounted;
 })();
 pending.set(name,operation);try{return(await operation).cleanup}finally{pending.delete(name)}
}
