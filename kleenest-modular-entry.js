...
(function(){'use strict';
 ...
 load('kleenest-map-session.js?session=5').then(()=>load('kleenest-map-preloader.js?preload=15')).then(()=>window.KleenestMapPreloader?.load?.()).catch(e=>console.warn('[Kleenest] eager Maps preload deferred',e));
 load('kleenest-app-shell.js?modular=20260815-shell36').catch(e=>{console.error('[Kleenest] shell bootstrap failed',e);const root=document.getElementById('modular-root');if(root)root.innerHTML='<div class="card" style="border-color:#c1503f"><b>Kleenest could not initialize.</b><p>Please refresh and try again.</p></div>';});
})();