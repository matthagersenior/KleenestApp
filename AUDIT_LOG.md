...
## 2026-08-16 — Platform quality/data contracts

- Added canonical platform quality-gate infrastructure for core initialization and runtime readiness checks.
- Added canonical dataset provenance/freshness classification infrastructure covering public, OSM/Overpass, government, business, community, system, and derived data.
- Kept provenance and quality contracts additive and non-destructive until each consumer is explicitly migrated.

## 2026-08-16 — Business feature-gap wiring

- Audited the canonical Business feature registry and confirmed Promotions, Events, Campaigns, Contests, QR Studio, Partner Programs, and advanced analytics are registered as advanced CRUD/analytics features.
- Identified a concrete gap: registration alone did not provide a canonical runtime CRUD implementation for those datasets.
- Added `cores/business/business-crud-core-v1.js` with real Supabase list/get/create/update/delete operations.
- Enforced business membership, tier/role entitlement, mutation-role restrictions, business scoping, record-id validation, editable-field filtering, and progression/metric side effects.
- Wired the Business runtime bridge to expose the canonical CRUD core alongside Business Value Core.
- Deliberately did not mark the Business feature family fully complete: each UI adapter still must consume the canonical CRUD runtime and each underlying table/RLS policy must be runtime-verified.

## Remaining P0/P1/P2 gates

- Complete public-data ingestion/prepopulation hardening and abuse/rate controls.
- Wire and verify every Business advanced feature UI to canonical CRUD.
- Verify every Business dataset against production schema/RLS and real create/read/update/delete behavior.
- Complete unified loading/error/empty-state adoption across all product cores.
- Finish removal of monolith-style startup/global dependency loading through lazy ownership.
- Complete device/browser regression coverage.
- Continue P1 location intelligence, Clean Route, Location Health, QR attribution, accessibility intelligence, admin integrity tooling, and abuse/reward protections.
- Continue P2 social, games, partner marketplace, enterprise APIs, fleet intelligence, and location-intelligence productization only after their underlying authority/data paths are verified.
