...
(function(){'use strict';
 ...
 async function query(q){for(const host of ['https://overpass-api.de/api/interpreter','https://overpass.kumi.systems/api/interpreter']){try{const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),25000);const res=await fetch(host+'?data='+encodeURIComponent(q),{headers:{Accept:'application/json'},signal:ctl.signal});clearTimeout(timer);if(!res.ok)continue;const data=await res.json();if(Array.isArray(data.elements))return data}catch(e){console.warn('[Kleenest] Overpass host failed',host,e)}}return {elements:[]};}
 function classify(t){
