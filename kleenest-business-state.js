...
(function(){'use strict';
 ...
 async function loadState(businessId,start,end){
  ...
  const [dashboardData,locs,reviewData,promos,members]=await Promise.all([data.dashboard(businessId,start,end),data.locations(businessId),data.recentReviews(businessId,50),data.promotions(businessId),data.memberships()]);
