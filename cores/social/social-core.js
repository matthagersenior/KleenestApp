...
export function createSocialCore({ supabase, mediaCore, root, user = null } = {}) {
  ...
  function subscribe() {
    ...
    const channel = supabase.channel(`social-core-${state.user.id}`).on('postgres_changes', { event: '*', schema: 'public', table: 'social_posts' }, () => render()).subscribe();
  ...
  async function render() {
    ...
    shell.innerHTML = `
      ...
      <nav class="social-core__nav" aria-label="Social">
        ${['feed','network','discover','compete','messages','notifications'].map(view => `<button type="button" data-view="${view}" aria-current="${state.view === view ? 'page' : 'false'}">${view[0].toUpperCase()+view.slice(1)}</button>`).join('')}
      </nav>
    ...
    if (!posts.length) { feed.innerHTML = '<p>No community posts yet. Be the first to share a discovery, tip, or win.</p>'; return; }
    feed.innerHTML = posts.map(post => `<article class="social-core__post" data-post-id="${escapeHtml(post.id)}"><div><strong>${escapeHtml(post.kind || 'Community post')}</strong></div><p>${escapeHtml(post.content || '')}</p>${post.media_url ? `<img src="${escapeAttribute(post.media_url)}" alt="Community media" loading="lazy">` : ''}</article>`).join('');
  }

  function openComposer() {
    const modal = document.createElement('dialog'); modal.className = 'social-core__dialog';
    modal.innerHTML = `<form method="dialog" class="social-core__composer"><h2>Share with the community</h2><select name="kind"><option value="discovery">Discovery</option><option value="tip">Tip</option><option value="verification">Verification</option><option value="review">Review</option><option value="route">Route</option><option value="win">Community win</option></select><textarea name="content" maxlength="2000" required placeholder="Share something useful with Kleenest…"></textarea><input name="media" type="file" accept="image/jpeg,image/png,image/webp"><button value="cancel">Cancel</button><button value="publish">Publish</button></form>`;
    document.body.append(modal); modal.showModal();
    modal.querySelector('form').addEventListener('submit', async event => { if (event.submitter?.value !== 'publish') return; event.preventDefault(); const form = event.currentTarget; const submit = form.querySelector('[value="publish"]'); submit.disabled = true; try { await createPost({ content: form.content.value, kind: form.kind.value, file: form.media.files[0] || null }); modal.close(); await render(); } catch (error) { submit.disabled = false; alert(error.message); } });
