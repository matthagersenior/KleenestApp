...
(function () {
  ...
  window.KleenestBusinessActions.run=async function(action,p){p=p||{};const A=window.KleenestBusinessActions;switch(action){case'inviteMember':return A.inviteMember(p.businessId,p.userId,p.role);case'createLocation':return A.createLocation(p.businessId,p.input||{});case'createPromotion':return A.createPromotion(p.businessId,p.input||{});case'updateLocation':return A.updateLocation(p.locationId,p.input||{});case'removeMember':return A.removeMember(p.businessId,p.userId);case'changeMemberRole':return A.changeMemberRole(p.businessId,p.userId,p.role);case'transferOwnership':return A.transferOwnership(p.businessId,p.newOwnerId);case'setPromotionActive':return A.setPromotionActive(p.promotionId,p.active);default:throw new Error('Unknown Kleenest business action: '+action);}};
  window.KleenestUI=window.KleenestUI||{};
  window.KleenestUI.runBusinessAction=window.KleenestBusinessActions.run;
})();
