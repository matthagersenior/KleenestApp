...
(function(){'use strict';
 ...
 async function query(q){const encoded=encodeURIComponent(q);for(const host of ['https://overpass-api.de/api/interpreter','https://overpass.kumi.systems/api/interpreter']){try{const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),15000);let res=await fetch(host+'?data='+encoded,{headers:{Accept:'application/json'},signal:ctl.signal,cache:'no-store'});clearTimeout(timer);if(!res.ok)continue;const data=await res.json();if(Array.isArray(data.elements))return data}catch(e){console.warn('[Kleenest] Overpass host/query failed',host,e)}}return {elements:[]};}
 function classify(t){
 ...
 const groups=[
  '["amenity"~"toilets|fuel|restaurant|fast_food|cafe|bar|pub|hospital|clinic|doctors|dentist|pharmacy"]',
 ...
 async function groupNearby(filter,r,lat,lng){const q=`[out:json][timeout:14];(nwr${filter}(around:${r},${lat},${lng}););out center tags;`;const data=await query(q);return data.elements||[];}
