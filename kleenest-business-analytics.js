...
(function(){'use strict';
 ...
 async function preferredSummary(start,end){const all=await current(start,end);return all.flatMap(x=>Array.isArray(x?.locations)?x.locations:[]);}
 async function preferredUsage(locationId,start,end){const rows=await preferredSummary(start,end);return rows.filter(r=>String(r.location_id)===String(locationId));}
 async function programUsage(programId,start,end){const all=await current(start,end);return all.flatMap(x=>Array.isArray(x?.programs)?x.programs:[]).filter(r=>String(r.program_id)===String(programId));}
