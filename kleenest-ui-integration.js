 ...
 * Explicit action hooks are supported, plus narrow capture-phase migrations
 * for existing check-in and review controls. The legacy UI stays intact;
...
(function () {
  ...
  document.addEventListener('click',async function(event){
    ...
    if(!el||el.dataset.kleenestMigrated==='1'||!window.KleenestRuntime?.supabaseReady?.()) return;
    ...
    try{
      const pos=await new Promise((resolve,reject)=>{if(!navigator.geolocation)return reject(new Error('Location is required for a live check-in.'));navigator.geolocation.getCurrentPosition(resolve,reject,{enableHighAccuracy:true,timeout:10000,maximumAge:60000});});
      const result=await KleenestRuntime.verifyCheckin(location.id,pos.coords.latitude,pos.coords.longitude);
      if(typeof KleenestRewards?.syncCheckin==='function'&&result?.id) await KleenestRewards.syncCheckin(result.id);
    ...
    }catch(error){emit('kleenest:action-error',{action:'checkin',error});alert(error?.message||'Live check-in could not be completed.');}
    finally{el.dataset.kleenestMigrated='0';setBusy(el,false);}
  },true);

  document.addEventListener('click',async function(event){
    const el=event.target.closest?.('[data-submit-review]');
    if(!el||el.dataset.kleenestMigrated==='1'||!window.KleenestRuntime?.supabaseReady?.()||typeof KleenestReviews?.create!=='function')return;
    const user=typeof currentUser==='function'?currentUser():null;const location=typeof selected!=='undefined'?selected:null;
    if(!user||!location?.id)return;
    const text=document.getElementById('review-text')?.value?.trim()||'';if(text.length<10)return;
    const stars=Number(typeof reviewStars!=='undefined'?reviewStars:5);const amenities=[...document.querySelectorAll('.rev-amenity:checked')].map(x=>x.value);
    event.preventDefault();event.stopImmediatePropagation();el.dataset.kleenestMigrated='1';setBusy(el,true);
    try{
      const last=typeof KleenestRewards?.lastCheckIn==='function'?KleenestRewards.lastCheckIn():null;
      const checkInId=last?.location_id===location.id?last.id:null;
      const cleanliness=Number.isFinite(Number(location.cleanlinessPct))&&Number(location.cleanlinessPct)>0?Number(location.cleanlinessPct):null;
      const created=await KleenestReviews.create(location.id,checkInId,stars,cleanliness,text);
      emit('kleenest:review-live',{review:created,locationId:location.id,checkInId,amenities});
      alert('✓ Review saved to Supabase.');if(typeof render==='function')render();
    }catch(error){emit('kleenest:action-error',{action:'review-create',error});alert(error?.message||'The review could not be saved to Supabase.');}
    finally{el.dataset.kleenestMigrated='0';setBusy(el,false);}
  },true);
